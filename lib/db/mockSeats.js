...
let _seats = [...Array.from({ length: 16 }).map((_, idx) => `stx_up_${idx}`), ...Array.from({ length: 16 }).map((_, idx) => `stx_low_${idx}`)];

_seats = _seats.map((seat, idx) => ({ id: seat, seat_type: seat.split("_")[1], seat_number: `${seat.split("_")[1]}-${idx}` }));
