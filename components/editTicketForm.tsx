...
const EditTicketForm = ({ setData, seat, user }) => {
  const { id: selectedSeatId, seat_number: selectedSeatNumber, seat_type: selectedSeatType, bus_id: selectedBusId } = seat || {};

  const { first_name: userFirstName, last_name: userLastName, email: userEmail, id: userId } = user || {};
  ...
  const onSubmit = handleSubmit((data) => {
    ...
    startTransition(async () => {
      const bookedTicketData = await editTicket(data);
      if (bookedTicketData?.userData) {
    ...
    <div className="flex h-full w-full flex-col justify-start gap-y-4">
      {user && (
        <div className="flex flex-col">
          <div className="text-xl text-tx-two underline underline-offset-2">Edit Ticket</div>
        </div>
      )}
      <div className="flex flex-col">
      ...
      <form onSubmit={onSubmit} className="mt-12 flex flex-col gap-y-4">
        <input type="hidden" id="user_id" value={userId} {...register("user_id")} />
